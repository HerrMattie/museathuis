...
// --- DEEL 3: BADGES ---
// Dit lost de BadgeGrid error op.
export const BADGE_ASSETS: Record<string, { label: string; desc: string; icon: string }> = {
    // START BADGES
    'welcome': { label: 'Eerste Stappen', desc: 'Je hebt je account aangemaakt.', icon: '👋' },
    'week_1': { label: 'Week 1', desc: 'Je bent 7 dagen op rij teruggekomen.', icon: '🔥' },
    
    // COLLECTIE BADGES
    'collector_5': { label: 'Verzamelaar', desc: '5 kunstwerken opgeslagen.', icon: '🖼️' },
    'collector_25': { label: 'Conservator', desc: '25 kunstwerken opgeslagen.', icon: '🏛️' },
    
    // SOCIAAL
    'salon_friend': { label: 'Vaste Gast', desc: 'Je hebt 10x de Salon bezocht.', icon: '☕' },
    'ambassador': { label: 'Ambassadeur', desc: 'Je hebt Premium weggegeven.', icon: '🎁' },
    
    // EXPLORATIE
    'flaneur': { label: 'Stadsflaneur', desc: 'Je hebt 3 verschillende steden bezocht in de tour.', icon: '🗺️' },
    'night_owl': { label: 'Nachtuil', desc: 'Je was online na 23:00 uur.', icon: '🦉' },
    'early_bird': { label: 'Vroege Vogel', desc: 'Je was online voor 07:00 uur.', icon: '🌅' },
    
    // LEVEL MIJLPAAL
    'level_10': { label: 'Verkenner', desc: 'Level 10 bereikt.', icon: '🧭' },
    'level_25': { label: 'Curator', desc: 'Level 25 bereikt.', icon: '💎' },
    'level_50': { label: 'Legende', desc: 'Het hoogste niveau bereikt.', icon: '👑' },
};

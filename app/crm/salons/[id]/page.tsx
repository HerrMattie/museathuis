...
export default async function EditSalonPage({ params }: { params: { id: string } }) {
  ...
  let initialItem = { id: params.id, title: '', short_description: '', content_markdown: '', status: 'draft' };
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-slate-800 mb-6">{params.id === 'new' ? 'Nieuwe Salon' : 'Salon Bewerken'}</h2>

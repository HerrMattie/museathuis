...
import { Headphones, ArrowRight, Clock, Calendar, ChevronLeft, ChevronRight, Lock } from 'lucide-react';
...
export default async function TourPage({ searchParams }: { searchParams: { date?: string } }) {
  ...
  // 2. Content
  const { data: schedule } = await supabase.from('dayprogram_schedule').select('tour_ids').eq('day_date', selectedDateStr).single();
  ...
  if (schedule?.tour_ids && schedule.tour_ids.length > 0) {
      ...
      const { data } = await supabase.from('tours').select('*').eq('status', 'published').limit(3);
      items = data || [];
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative py-12 mb-8 border-b border-white/10">
             ...
             <div className="relative z-10">
                <p className="text-museum-gold text-sm font-bold uppercase tracking-[0.2em] mb-2">Luister & Ontdek</p>
                <h1 className="text-5xl font-serif font-black text-white">Audiotours</h1>
             </div>
        </div>

        {/* Datum Navigatie */}
        <div className="flex items-center justify-between mb-8 bg-white/5 p-4 rounded-xl border border-white/10">
            <Link href={`/tour?date=${prevStr}`} className="flex items-center gap-2 text-sm font-bold hover:text-museum-gold transition-colors">
            ...
            {!isToday ? (
                <Link href={`/tour?date=${nextStr}`} className="flex items-center gap-2 text-sm font-bold hover:text-museum-gold transition-colors">
        ...
        {isLocked ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {items.map((tour) => (
                    <Link key={tour.id} href={`/tour/${tour.id}`} className="group bg-midnight-900 border border-white/10 rounded-2xl overflow-hidden hover:border-museum-gold/40 transition-all hover:-translate-y-2 hover:shadow-2xl flex flex-col">
                        <div className="h-64 relative overflow-hidden bg-black">
                            <img src={tour.hero_image_url} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90"/>
                            <div className={`absolute top-4 left-4 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest text-white border border-white/10 z-10 ${tour.is_premium ? 'bg-museum-gold text-black' : 'bg-black/50 backdrop-blur-md'}`}>
                                {tour.is_premium ? 'Premium' : 'Gratis'}
                            </div>
                            <div className="absolute top-4 right-4 z-20"><LikeButton itemId={tour.id} itemType="tour" userId={user?.id} /></div>
                        </div>
                        <div className="p-8 flex-1 flex flex-col">
                            ...
                            <p className="text-gray-400 text-sm line-clamp-3 mb-6 leading-relaxed flex-1">{tour.intro}</p>
                ...
                {items.length === 0 && <p className="text-gray-500 col-span-3 text-center py-10">Geen tours voor deze datum.</p>}
            </div>
        )}
      </div>
    </div>

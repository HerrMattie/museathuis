...
export default async function SalonPage() {
  ...
  // Haal alleen gepubliceerde salons op
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative py-12 mb-12 border-b border-white/10">
             ...
             <div className="relative z-10">
                <p className="text-museum-gold text-sm font-bold uppercase tracking-[0.2em] mb-3">Curated Collections</p>
                ...
                <p className="text-xl text-gray-300 max-w-2xl leading-relaxed font-light">
                    Exclusieve thema-collecties samengesteld door onze curatoren.
                </p>
             </div>
        </div>
                    ...
                    <Link key={salon.id} href={isLocked ? '/pricing' : `/salon/${salon.id}`} className="group bg-midnight-900 border border-white/10 rounded-2xl overflow-hidden hover:border-museum-gold/40 transition-all hover:-translate-y-2 hover:shadow-2xl flex flex-col h-full">
                        <div className="h-64 relative overflow-hidden bg-black">
                            ...
                            <div className={`absolute top-4 left-4 px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest text-white border border-white/10 shadow-lg ${isLocked ? 'bg-black/80' : 'bg-museum-gold text-black'}`}>
                                {isLocked ? <span className="flex items-center gap-1"><Lock size={10}/> Premium</span> : 'Collectie'}
                            </div>
                        </div>
                        ...
                        <div className="p-8 flex-1 flex flex-col">
                            ...
                            <div className="mt-auto pt-4 border-t border-white/5 flex justify-between items-center">
                                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Bekijk Collectie</span>
        ...
        {(!salons || salons.length === 0) && (
            <div className="text-center py-20 bg-white/5 rounded-2xl border border-dashed border-white/10">
                ...
                <p className="text-gray-400">Er zijn momenteel geen salons geopend.</p>
            </div>
        )}

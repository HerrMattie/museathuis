      ...
      <div className="grid grid-cols-1 md:grid-cols-12 md:grid-rows-2 gap-6 h-auto md:h-[600px]">
        ...
        <Link href={tour ? `/tour/${tour.id}` : '#'} className="group relative col-span-1 md:col-span-8 md:row-span-2 bg-midnight-900 rounded-3xl border border-white/5 overflow-hidden hover:border-museum-gold/30 transition-all">
          ...
          <div className="absolute bottom-0 left-0 p-8 w-full">
            ...
            <h2 className="font-serif text-3xl md:text-4xl text-white font-bold mb-3">{tour?.title || 'Binnenkort verwacht'}</h2>
            <p className="text-gray-200 line-clamp-2 max-w-xl mb-6 font-light">{tour?.intro || 'Onze curatoren zijn bezig.'}</p>
            {tour && (
        ...
        {/* GAME CARD (Klein) */}
        <Link href={game ? `/game/${game.id}` : '#'} className="col-span-1 md:col-span-4 bg-midnight-900 rounded-3xl border border-white/5 p-6 hover:bg-midnight-800 transition-all flex flex-col justify-between group">
        ...
        {/* FOCUS CARD (Klein) */}
        <Link href={focus ? `/focus/${focus.id}` : '#'} className="col-span-1 md:col-span-4 bg-midnight-900 rounded-3xl border border-white/5 p-6 hover:bg-midnight-800 transition-all flex flex-col justify-between relative overflow-hidden group">

...
export default async function FocusPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="relative h-screen w-full bg-black overflow-hidden flex flex-col md:flex-row">
      ...
      {/* TEKST PANEEL */}
      <div className="relative w-full md:w-1/4 h-1/2 md:h-full bg-midnight-950 flex flex-col p-8 md:p-12 overflow-y-auto">
        <Link href="/" className="absolute top-6 right-6 p-2 bg-white/10 rounded-full text-white hover:bg-white/20">
          <X size={24} />
        </Link>

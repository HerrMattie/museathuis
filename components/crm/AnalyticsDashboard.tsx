...
export default function AnalyticsDashboard() {
    ...
    const analyzeData = async () => {
        ...
        // 3. Opleiding vs Premium (CORRECTIE: Basis vs Premium splitsen)
        ...
        const eduData = eduLevels.map(level => {
            ...
            const basis = group.length - premium; // Dit voorkomt dubbeltelling!
            return { 
                name: level, 
                Basis: basis, // Gratis leden
                Premium: premium, // Betalende leden
                Totaal: group.length // Voor berekeningen, niet voor de grafiekstapeling
            };
        });

        // 3b. Leeftijd vs Premium (CORRECTIE: Ook hier splitsen)
        ...
        const ageData = ageGroups.map(group => {
            ...
            const basisCount = groupUsers.length - premiumCount;
            return {
                name: group,
                Basis: basisCount,
        ...
        logs.forEach(log => {
            ...
            if (log.action_type === '404_visit') errorCount++;
            
            if (log.action_type === 'complete_game') {
                if (meta.score >= 100 && meta.duration < 10) suspiciousGames++;
            }
        ...
        // --- DATA OPSLAAN ---
        // Beste leeftijd bepalen voor insights
        const bestConvertingAge = ageData.reduce((prev, current) => (prev.Conversie > current.Conversie) ? prev : current);

        setData({
            timeData, geoData, eduData, ageData, deviceData, behaviorData, radarData, engagementData,
            insights: {
    ...
    if (loading) return <div className="p-12 text-center text-slate-400"><Loader2 className="animate-spin inline mr-2"/>Analyseren van data...</div>;
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
                    ...
                    <p className="text-xs text-red-500 mt-2">Games &lt; 10 sec.</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <p className="text-xs text-green-600 mt-2">Hoogste betrokkenheid.</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="flex items-end gap-2">
                        ...
                        <span className="text-sm text-slate-500 mb-1">404 meldingen</span>
                    </div>
                    <p className="text-xs text-slate-400 mt-2">Dode links.</p>
                </div>
            </div>

            {/* RIJ 2: DEMOGRAFIE (Gecorrigeerd!) */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                
                {/* Leeftijd Grafiek (AANGEPAST) */}
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <h3 className="font-bold text-slate-800 mb-6">Leden & Premium per Leeftijd</h3>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data.ageData}>
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="Basis" stackId="a" fill="#cbd5e1" name="Gratis Leden" radius={[0, 0, 4, 4]} />
                                <Bar dataKey="Premium" stackId="a" fill="#D4AF37" name="Premium Leden" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>

                {/* Opleiding Grafiek (AANGEPAST) */}
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
                        <GraduationCap size={18} className="text-green-600"/> Opleiding Verdeling
                    </h3>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data.eduData}>
                                ...
                                <Legend />
                                <Bar dataKey="Basis" stackId="a" fill="#e2e8f0" name="Gratis" />
                                <Bar dataKey="Premium" stackId="a" fill="#D4AF37" name="Premium" />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
            ...
            {/* RIJ 3: CONTENT & GEDRAG */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2">
                        <Clock size={18} className="text-blue-500"/> Betrokkenheid (Duur)
                    </h3>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
            ...
            {/* RIJ 4: TIJD & LOCATIE & RADAR */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2">
                        <Clock size={18} className="text-museum-gold"/> Piekuren
                    </h3>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2">
                        <Lightbulb size={18} className="text-yellow-500"/> Smaak Radar
                    </h3>
                    <div className="h-[250px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
